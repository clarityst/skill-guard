...
function installIntoTemp({ slug, version, workdirTmp }) {
  ...
  if (process.env.SKILL_GUARD_DEBUG) {
  ...
  if (r.status !== 0) {
    ...
    // If registry throttles us, don't fail the whole run — just skip this slug.
    if (out.toLowerCase().includes("rate limit")) {
      console.log(`\n=== ${slug} ===\nSKIP: clawhub rate limited (cannot fetch latest right now)`);
      return { skipped: true, reason: "rate_limit", out };
    }
...
function getLatestVersionViaInspect(slug) {
  const { cmd, prefixArgs } = findClawhubCmd();
  const r = run(cmd, [...prefixArgs, "inspect", "--json", slug]);

  if (r.status !== 0) {
    throw new Error(
      `clawhub inspect failed.\n` +
        `status: ${r.status}\n` +
  ...
  const v =
    ...
    data?.skill?.version ??
    data?.latest?.version ??
    (Array.isArray(data?.versions) ? data.versions[0]?.version : undefined);

  if (!v || typeof v !== "string") {
    throw new Error(`Could not determine latest version from inspect JSON.\n${r.stdout}`);
  }
...
program
  ...
  .action((opts) => {
    ...
    for (const slug of slugs) {
      ...
      const installRes = installIntoTemp({ slug, version: versionNew, workdirTmp: tmp });
if (installRes?.skipped) {
  continue;
}
